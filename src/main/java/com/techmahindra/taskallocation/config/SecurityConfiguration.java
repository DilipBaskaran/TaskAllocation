 ...
 * http.csrf().disable();
 * http.authorizeRequests().antMatchers("/","/home","/login",
 * "/logout").permitAll(); http.authorizeRequests().antMatchers("/cart").access(
 * "hasAnyRole('ROLE_USER')");
 * http.authorizeRequests().and().exceptionHandling().accessDeniedPage("/403");
 * http.authorizeRequests().and().httpBasic() .and().logout(); }
 * 
 ...
 * throws Exception { //System.out.println(passwordEncoder().encode("test"));
